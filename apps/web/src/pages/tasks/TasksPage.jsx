...
import {
  ...
  Tooltip, message, Typography, Space, Slider
} from 'antd'
import {
...
function TasksPage() {
  ...
  const handleDelete = async (id) => {
    ...
    if (error) messageApi.error('Gagal hapus task!')
      ...
      messageApi.success('Task dihapus!')
      fetchTasks()
    }
  }
      ...
      render: (_, record) => (
        <Space>
          <Tooltip title={record.status === 'done' ? 'Tandai belum selesai' : 'Tandai selesai'}>
            <Button
              size="small"
              type={record.status === 'done' ? 'default' : 'primary'}
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ marginTop: 16 }}>
          <Form.Item
            name="title"
            label="Nama Task"
            rules={[{ required: true, message: 'Nama task tidak boleh kosong' }]}
          >
